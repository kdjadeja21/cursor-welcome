import { chromium } from "playwright";
import { mkdir, rename } from "fs/promises";

const OUTPUT_DIR = "/opt/cursor/artifacts/videos";
const FINAL_WEBM = "/opt/cursor/artifacts/videos/cursor-welcome-demo.webm";
const VIEWPORT = { width: 1280, height: 720 };

async function injectDemoStyles(page) {
  await page.evaluate(() => {
    if (document.getElementById("demo-highlight-styles")) return;

    const style = document.createElement("style");
    style.id = "demo-highlight-styles";
    style.textContent = `
      .demo-zoom-target {
        transition: transform 0.75s cubic-bezier(0.22, 1, 0.36, 1);
        transform-origin: 0 0;
        will-change: transform;
      }
      @keyframes demo-pulse-ring {
        0%, 100% { box-shadow: 0 0 0 2px rgba(245, 78, 0, 0.9), 0 0 16px rgba(245, 78, 0, 0.35); }
        50% { box-shadow: 0 0 0 6px rgba(245, 78, 0, 0.45), 0 0 28px rgba(245, 78, 0, 0.55); }
      }
      @keyframes demo-click-ripple {
        0% { transform: translate(-50%, -50%) scale(0.35); opacity: 1; }
        100% { transform: translate(-50%, -50%) scale(2.4); opacity: 0; }
      }
      @keyframes demo-pointer-bob {
        0%, 100% { transform: translate(-5px, -5px) rotate(-8deg); }
        50% { transform: translate(0, 0) rotate(-8deg); }
      }
      @keyframes demo-arrow-draw {
        to { stroke-dashoffset: 0; }
      }
      @keyframes demo-arrow-head-pop {
        0% { opacity: 0; transform: scale(0.4); }
        65% { opacity: 1; transform: scale(1.2); }
        100% { opacity: 1; transform: scale(1); }
      }
      @keyframes demo-arrow-bounce {
        0%, 100% { transform: translate(0, 0); }
        50% { transform: translate(var(--bounce-x, 0px), var(--bounce-y, 0px)); }
      }
      .demo-highlight-ring {
        position: fixed;
        border: 2px solid #f54e00;
        border-radius: 10px;
        animation: demo-pulse-ring 1s ease-in-out infinite;
        pointer-events: none;
        z-index: 99998;
        box-sizing: border-box;
      }
      .demo-pointer {
        position: fixed;
        width: 34px;
        height: 34px;
        z-index: 100001;
        pointer-events: none;
        transform: rotate(-8deg);
        filter: drop-shadow(0 3px 8px rgba(0, 0, 0, 0.55));
        animation: demo-pointer-bob 0.85s ease-in-out infinite;
      }
      .demo-click-ripple {
        position: fixed;
        width: 48px;
        height: 48px;
        border: 3px solid #f54e00;
        border-radius: 50%;
        animation: demo-click-ripple 0.45s ease-out forwards;
        pointer-events: none;
        z-index: 100000;
      }
      #demo-highlights svg {
        position: fixed;
        inset: 0;
        width: 100%;
        height: 100%;
        pointer-events: none;
        z-index: 99997;
        overflow: visible;
      }
      #demo-highlights .demo-arrow-path {
        stroke: #f54e00;
        stroke-width: 3.5;
        fill: none;
        stroke-linecap: round;
        stroke-dasharray: var(--arrow-length);
        stroke-dashoffset: var(--arrow-length);
        animation: demo-arrow-draw 0.65s ease-out forwards;
      }
      #demo-highlights .demo-arrow-head {
        fill: #f54e00;
        transform-box: fill-box;
        transform-origin: center;
        animation: demo-arrow-head-pop 0.45s ease-out 0.5s forwards;
        opacity: 0;
      }
      #demo-highlights .demo-arrow-head-group {
        animation: demo-arrow-bounce 1.1s ease-in-out 0.9s infinite;
      }
    `;
    document.head.appendChild(style);
  });
}

async function tagZoomTargets(page) {
  await page.evaluate(() => {
    document
      .querySelectorAll(
        ".welcome-root, .sidebar-panel, .sidebar-backdrop, .welcome-footer",
      )
      .forEach((node) => node.classList.add("demo-zoom-target"));

    const actionBar = document.querySelector(".fixed.right-5.top-5");
    if (actionBar) actionBar.classList.add("demo-action-bar", "demo-zoom-target");
  });
}

async function resetZoom(page) {
  await page.evaluate(() => {
    document.querySelectorAll(".demo-zoom-target").forEach((node) => {
      node.style.transform = "";
    });
  });
  await page.waitForTimeout(280);
}

async function zoomToBox(page, box, scale = 1.5) {
  const cx = box.x + box.width / 2;
  const cy = box.y + box.height / 2;

  await page.evaluate(
    ({ cx, cy, scale, vw, vh }) => {
      const tx = vw / 2 - cx * scale;
      const ty = vh / 2 - cy * scale;
      const transform = `translate(${tx}px, ${ty}px) scale(${scale})`;
      document.querySelectorAll(".demo-zoom-target").forEach((node) => {
        node.style.transform = transform;
      });
    },
    { cx, cy, scale, vw: VIEWPORT.width, vh: VIEWPORT.height },
  );

  await page.waitForTimeout(780);
}

async function clearHighlights(page) {
  await page.evaluate(() => {
    document.getElementById("demo-highlights")?.remove();
    document.getElementById("demo-pointer")?.remove();
    document.querySelectorAll(".demo-click-ripple").forEach((node) => node.remove());
  });
}

async function drawAnimatedArrow(page, box, from = "auto") {
  await injectDemoStyles(page);

  await page.evaluate(
    ({ x, y, w, h, from }) => {
      document.getElementById("demo-highlights")?.remove();

      const container = document.createElement("div");
      container.id = "demo-highlights";

      const ring = document.createElement("div");
      ring.className = "demo-highlight-ring";
      ring.style.left = `${x - 6}px`;
      ring.style.top = `${y - 6}px`;
      ring.style.width = `${w + 12}px`;
      ring.style.height = `${h + 12}px`;
      container.appendChild(ring);

      const cx = x + w / 2;
      const cy = y + h / 2;
      const margin = 20;

      let startX;
      let startY;

      if (from === "left") {
        startX = x - 110;
        startY = cy;
      } else if (from === "right") {
        startX = x + w + 110;
        startY = cy;
      } else if (from === "top") {
        startX = cx;
        startY = y - 85;
      } else if (from === "bottom") {
        startX = cx;
        startY = y + h + 85;
      } else {
        const spaceLeft = x;
        const spaceRight = window.innerWidth - (x + w);
        const spaceTop = y;
        const spaceBottom = window.innerHeight - (y + h);

        if (
          spaceLeft >= spaceRight &&
          spaceLeft >= spaceTop &&
          spaceLeft >= spaceBottom
        ) {
          startX = x - 110;
          startY = cy;
        } else if (spaceRight >= spaceTop && spaceRight >= spaceBottom) {
          startX = x + w + 110;
          startY = cy;
        } else if (spaceTop >= spaceBottom) {
          startX = cx;
          startY = y - 85;
        } else {
          startX = cx;
          startY = y + h + 85;
        }
      }

      const dx = cx - startX;
      const dy = cy - startY;
      const length = Math.hypot(dx, dy) || 1;
      const tipX = cx - (dx / length) * margin;
      const tipY = cy - (dy / length) * margin;
      const curveX = (startX + tipX) / 2 + (dy / length) * 28;
      const curveY = (startY + tipY) / 2 - (dx / length) * 28;
      const pathLength = Math.hypot(tipX - startX, tipY - startY) * 1.15;

      const angle = Math.atan2(tipY - curveY, tipX - curveX);
      const headLength = 16;
      const headAngle = Math.PI / 7;
      const leftX = tipX - headLength * Math.cos(angle - headAngle);
      const leftY = tipY - headLength * Math.sin(angle - headAngle);
      const rightX = tipX - headLength * Math.cos(angle + headAngle);
      const rightY = tipY - headLength * Math.sin(angle + headAngle);
      const bounceX = `${Math.cos(angle) * 4}px`;
      const bounceY = `${Math.sin(angle) * 4}px`;

      const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      svg.innerHTML = `
        <path
          class="demo-arrow-path"
          d="M ${startX} ${startY} Q ${curveX} ${curveY} ${tipX} ${tipY}"
          style="--arrow-length: ${pathLength}"
        />
        <g class="demo-arrow-head-group" style="--bounce-x: ${bounceX}; --bounce-y: ${bounceY};">
          <polygon class="demo-arrow-head" points="${tipX},${tipY} ${leftX},${leftY} ${rightX},${rightY}" />
        </g>
      `;
      container.appendChild(svg);
      document.body.appendChild(container);
    },
    { x: box.x, y: box.y, w: box.width, h: box.height, from },
  );
}

async function focusOn(page, locator, options = {}) {
  const scale = options.scale ?? 1.5;

  await resetZoom(page);
  await clearHighlights(page);

  const box = await locator.boundingBox();
  if (!box) return null;

  await zoomToBox(page, box, scale);

  const zoomedBox = await locator.boundingBox();
  if (!zoomedBox) return null;

  await drawAnimatedArrow(page, zoomedBox, options.from ?? "auto");
  return zoomedBox;
}

async function movePointer(page, x, y, animate = true) {
  await injectDemoStyles(page);

  await page.evaluate(
    ({ x, y, animate }) => {
      let pointer = document.getElementById("demo-pointer");
      if (!pointer) {
        pointer = document.createElement("div");
        pointer.id = "demo-pointer";
        pointer.className = "demo-pointer";
        pointer.innerHTML = `
          <svg width="34" height="34" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 2 L4 26 L11 19 L16 28 L20 26 L15 17 L26 17 Z" fill="#ffffff" stroke="#14120b" stroke-width="1.5" stroke-linejoin="round"/>
          </svg>
        `;
        document.body.appendChild(pointer);
      }

      pointer.style.transition = animate ? "left 0.55s ease-out, top 0.55s ease-out" : "none";
      pointer.style.left = `${x - 4}px`;
      pointer.style.top = `${y - 2}px`;
    },
    { x, y, animate },
  );
}

async function showClickRipple(page, x, y) {
  await page.evaluate(({ x, y }) => {
    const ripple = document.createElement("div");
    ripple.className = "demo-click-ripple";
    ripple.style.left = `${x}px`;
    ripple.style.top = `${y}px`;
    document.body.appendChild(ripple);
    window.setTimeout(() => ripple.remove(), 500);
  }, { x, y });
}

async function demoClick(page, locator, options = {}) {
  const zoomedBox = await focusOn(page, locator, options);
  if (!zoomedBox) {
    await locator.click();
    return;
  }

  const targetX = zoomedBox.x + zoomedBox.width / 2;
  const targetY = zoomedBox.y + zoomedBox.height / 2;
  const startX = Math.max(120, zoomedBox.x - 120);
  const startY = zoomedBox.y + zoomedBox.height + 70;

  await page.waitForTimeout(650);
  await movePointer(page, startX, startY, false);
  await page.waitForTimeout(120);
  await movePointer(page, targetX, targetY, true);
  await page.waitForTimeout(600);
  await showClickRipple(page, targetX, targetY);
  await page.waitForTimeout(220);

  await clearHighlights(page);
  await resetZoom(page);
  await locator.click();
  await page.waitForTimeout(350);
}

async function demoFill(page, locator, value, options = {}) {
  const zoomedBox = await focusOn(page, locator, options);
  if (!zoomedBox) {
    await locator.fill(value);
    return;
  }

  await page.waitForTimeout(900);
  await clearHighlights(page);
  await resetZoom(page);
  await locator.fill(value);
  await page.waitForTimeout(450);
}

async function showStep(page, stepNum, title, subtitle = "") {
  await page.evaluate(
    ({ stepNum, title, subtitle }) => {
      let overlay = document.getElementById("demo-step-overlay");
      if (!overlay) {
        overlay = document.createElement("div");
        overlay.id = "demo-step-overlay";
        overlay.style.cssText = `
          position: fixed;
          top: 20px;
          left: 20px;
          z-index: 100002;
          background: rgba(20, 18, 11, 0.92);
          border: 1px solid rgba(245, 78, 0, 0.35);
          border-radius: 10px;
          padding: 12px 16px;
          width: min(300px, calc(100vw - 220px));
          text-align: left;
          font-family: ui-sans-serif, system-ui, sans-serif;
          box-shadow: 0 8px 24px rgba(0, 0, 0, 0.35);
          pointer-events: none;
        `;
        document.body.appendChild(overlay);
      }

      overlay.innerHTML = `
        <div style="color: #f54e00; font-size: 10px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; margin-bottom: 6px;">
          Step ${stepNum}
        </div>
        <div style="color: #edecec; font-size: 14px; font-weight: 600; line-height: 1.3; margin-bottom: ${subtitle ? "4px" : "0"};">
          ${title}
        </div>
        ${
          subtitle
            ? `<div style="color: rgba(237, 236, 236, 0.62); font-size: 12px; line-height: 1.35;">${subtitle}</div>`
            : ""
        }
      `;
    },
    { stepNum, title, subtitle },
  );
}

await mkdir(OUTPUT_DIR, { recursive: true });

const browser = await chromium.launch({ headless: true });
const context = await browser.newContext({
  viewport: VIEWPORT,
  recordVideo: {
    dir: OUTPUT_DIR,
    size: VIEWPORT,
  },
});

const page = await context.newPage();

try {
  await page.addInitScript(() => {
    localStorage.removeItem("cursor-welcome-config");
  });

  await page.goto("http://localhost:3000", { waitUntil: "networkidle" });
  await tagZoomTargets(page);

  await showStep(
    page,
    1,
    "Open the welcome screen",
    "Run npm run dev, then visit http://localhost:3000 in your browser",
  );
  await page.waitForTimeout(2200);

  await showStep(
    page,
    2,
    "Watch the welcome animation",
    "Logo, brand, title, date, and sponsors animate in automatically",
  );
  await focusOn(page, page.locator(".welcome-logo-wrap"), { from: "left", scale: 1.35 });
  await page.waitForTimeout(2600);
  await focusOn(page, page.locator(".welcome-heading"), { from: "bottom", scale: 1.28 });
  await page.waitForTimeout(2600);
  await clearHighlights(page);
  await resetZoom(page);
  await page.waitForTimeout(1600);

  await showStep(
    page,
    3,
    "Click Edit to customize",
    "Use the Edit button (top-right) or press E on your keyboard",
  );
  await page.waitForTimeout(900);
  await demoClick(page, page.getByRole("button", { name: "Edit" }), {
    from: "left",
    scale: 1.7,
  });
  await page.locator(".sidebar-panel").waitFor({ state: "visible" });

  await showStep(
    page,
    4,
    "Update your event details",
    "Change brand, title, date, replay interval, and add up to 5 sponsors",
  );
  await page.waitForTimeout(700);

  await demoFill(page, page.getByPlaceholder("Cursor Ahmedabad"), "Cursor San Francisco", {
    from: "left",
    scale: 1.6,
  });

  await demoFill(
    page,
    page.getByPlaceholder("Welcome to the Cursor Community Workshop"),
    "Welcome to the Cursor Community Meetup",
    { from: "left", scale: 1.5 },
  );

  await demoClick(page, page.getByRole("button", { name: "Add Sponsor" }), {
    from: "left",
    scale: 1.65,
  });

  await demoFill(
    page,
    page.getByPlaceholder("Title (e.g. Gold Sponsor)"),
    "Hosted by",
    { from: "left", scale: 1.65 },
  );

  await demoFill(
    page,
    page.getByPlaceholder("Name (e.g. Acme Corp)"),
    "Cursor Team",
    { from: "left", scale: 1.65 },
  );

  await showStep(
    page,
    5,
    "Close the editor",
    "Changes apply live and are saved automatically in your browser",
  );
  await page.waitForTimeout(800);
  await demoClick(page, page.getByRole("button", { name: "Close", exact: true }), {
    from: "left",
    scale: 1.7,
  });
  await page.waitForTimeout(4500);

  await showStep(
    page,
    6,
    "Enter presentation mode",
    "Click Preview (or press F / P) for a fullscreen display on projectors and TVs",
  );
  await page.waitForTimeout(900);
  await demoClick(page, page.getByRole("button", { name: "Preview" }), {
    from: "left",
    scale: 1.7,
  });
  await page.waitForTimeout(2600);
  await page.keyboard.press("Escape");
  await clearHighlights(page);
  await resetZoom(page);
  await page.waitForTimeout(800);

  await showStep(
    page,
    7,
    "Share your configuration",
    "Click Share to copy a link — open it on another device to load the same setup",
  );
  await page.waitForTimeout(900);
  await demoClick(page, page.getByRole("button", { name: "Share" }), {
    from: "left",
    scale: 1.7,
  });
  await page.waitForTimeout(1800);

  await showStep(
    page,
    8,
    "Reset anytime",
    "Open Edit → Reset to defaults to clear saved settings and start over",
  );
  await page.waitForTimeout(900);
  await demoClick(page, page.getByRole("button", { name: "Edit" }), {
    from: "left",
    scale: 1.7,
  });
  await page.locator(".sidebar-panel").waitFor({ state: "visible" });
  await focusOn(page, page.getByRole("button", { name: "Reset to defaults" }), {
    from: "left",
    scale: 1.6,
  });
  await page.waitForTimeout(2600);
  await clearHighlights(page);
  await resetZoom(page);
} finally {
  const video = page.video();
  await page.close();
  await context.close();
  await browser.close();

  if (video) {
    const tempPath = await video.path();
    await rename(tempPath, FINAL_WEBM);
    console.log(`Demo saved to ${FINAL_WEBM}`);
  }
}
