...
export function buildVideoSegmentArgs(options: {
  ...
  const vf = [
    buildScalePadFilter(options.video),
    // Freeze the last frame when the clip is shorter than narration.
    `tpad=stop_mode=clone:stop_duration=${options.durationSec.toFixed(3)}`,
  ].join(",");
  return [
    "-y",
    "-i",
