...
import { followLook, TOUR_TIME, tourLook } from "./bot/gaze";
...
export function GrokBotAvatar({
  ...
  const cycle = useMemo<Block[]>(() => defaultCycle().blocks, []);
  ...
  useEffect(() => {
    ...
    const tick = (ms: number) => {
      ...
      if (!primed) {
        engine.reset("idle", 0);
        engine.setLook(tourLook(0), clock - SCRIPT_MORPH, SCRIPT_MORPH);
        primed = true;
      }

      if (clock < TOUR_TIME) {
        scriptGaze(tourLook(clock));
      } else {
        if (!cycleStarted) {
          cycleStarted = true;
          engine.setLook(null, clock);
